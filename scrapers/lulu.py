from config import headers_lulu, baseUrls, useragents
from bs4 import BeautifulSoup
import requests
import json
import random

def get_listing_lulu(keyword):
    products_data = []
    final_url = baseUrls['lulu'] + keyword
    headers_lulu["User-Agent"] = useragents[random.randint(0,31)]
    response = requests.get(final_url, headers=headers_lulu, timeout=10)
